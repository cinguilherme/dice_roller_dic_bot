...
class TestGetDificulty(TestCase):
    ...
    def test_with_missing_dificulty_use_default_6(self):
...
class TestCritBalance(TestCase):
    ...
    def test_crit_balance_calculation(self):
        ...
        check = roll_functions.critical_balance(
            success=[9, 10], crit_success=[10], crit_failures=[1])
        assert(check == 1)
        check = roll_functions.critical_balance(
            success=[9, 10, 10], crit_success=[10, 10], crit_failures=[1])
        assert(check == 3)
        check = roll_functions.critical_balance(
            success=[9, 9], crit_success=[], crit_failures=[1])
        assert(check == 1)
